...
Then start playback with the green playback toggle in the time subpatch. Play some chords and you should hear the generated notes sound on the connected synthesizer. Engage latch mode if needed. If you have a nice pattern going, click the loop toggle in the panel to engage the looper, fire up another Raptor instance and play along. E.g., you may want to try the tr808 preset (preset #10 on the quick dial) to get a simple drum pattern going.
...
- Step 2': Click or wiggle the control in the time, looper, or panel subpatch.
...
The [Novation Launchkey][] is Novation's keyboard controller which comes in different sizes, including a compact and budget-friendly Mini version. It is arguably the best Ableton Live controller with keys at an affordable price. The Launchkey combines a standard MIDI keyboard with eight encoders, nine faders (on the larger models), and a Launchpad-like 2x8 grid of pads. In terms of versatility, the Launchkey is right up there with the Launchpad, and it makes up for the smaller grid with its physical keys and knobs, and the LCD display.
