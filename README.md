...
The transport buttons also work as expected, and you can select Raptor instances and switch presets with the arrow buttons like on the Launchpad. There's also a strip of pads for selecting Raptor instances which appears if you press and hold the *Device Select* button. Information about the currently selected instance and preset can be shown on the little LCD screen with the *Device Lock* button. (Note that these two buttons and the LCD screen are not available on the Launchkey Mini.)
