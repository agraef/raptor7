...
In DAW mode (if the DAW ports are connected to Pd and the Launchkey driver is turned on), the Launchkey becomes a little Launchpad with mappable pads, drum grid, and knobs for controlling five banks of Raptor controls; please check the map file for details. To these ends, the Launchkey can be switched into various different modes by pressing the Shift key together with one of the pads. The driver supports the Session and Drum pad modes, as well as the Device, Volume, Pan, and Send A+B knob modes. By default, the device starts up with the Session and Volume modes activated. (The fader modes are not supported at present, and there are no ready-made bindings for them in the provided MIDI map either. But of course you can just map the faders yourself using MIDI learn if needed.)

The transport buttons also work as expected, and you can select Raptor instances and switch presets with the arrow buttons like on the Launchpad. There's also a strip of pads for selecting Raptor instances like on the Launchpad Pro, which appears if you press and hold the *Device Select* button. Information about the currently selected instance can be shown on the little LCD screen by clicking the *Device Lock* button. Note that the Device Select/Lock buttons and the LCD screen are only available on the larger models (Launchkey 25 and up), not on the Launchkey Mini.
