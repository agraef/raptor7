...
Probably the most annoying quirk in Raptor's controller support is that when Raptor is about to exit, it still tries to send some MIDI data to reset certain devices to their default state. If that doesn't work then some buttons may stay lit, or the device may remain stuck in a special DAW or session mode. Chances are that you noticed this if you just quit Pd after using Raptor with the Hercules DJ Control or the Novation Launchpad and Launchkey controllers. Raptor tries its best to avert this issue by sending all finalization data to receivers in the main patch before it terminates. But even that won't help if you just exit Pd without closing all toplevel Raptor patches first, so you'll just have to remember to do that.

A much less frequent issue is that devices may only be partially initialized after startup. In particular, I noticed this with the Launchkey session grid on Linux with ALSA and Pipewire. Again, Raptor goes out of its way to work around this by delaying parts of the device initialization, but it still happens on occasion. As a remedy for this mishap, just close the patch and launch it again, it usually works the second time.
