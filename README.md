...
If the Launchkey driver is enabled, it switches the device to DAW mode, which unlocks the Launchpad-like features including mappable pads, drum grid, and knobs and faders for controlling five banks of Raptor controls. To these ends, the Launchkey can be switched into various different modes by pressing the Shift key together with one of the pads or the track select buttons. The driver supports the Session and Drum pad modes, as well as the Device, Volume, Pan, and Send A+B knob and fader modes. By default, the device starts up in the Session (pad), Volume (knob), and Device (fader) modes. With the larger models (Launchkey 49 and up), you get the all-important *ninth fader* as a bonus; Raptor has this hardwired to CC7 (volume), so that you can use this fader for controlling the volume of your synth, no matter which fader mode you're in.

The transport buttons also work as expected, and you can select Raptor instances and switch presets with the arrow buttons like on the Launchpad. There's also a strip of pads for selecting Raptor instances which appears if you press and hold the *Device Select* button. On the Launchkey 49 MK3 and up, you can also use the track select buttons for this purpose. Information about the currently selected instance and preset can be shown on the little LCD screen with the *Device Lock* button. (Note that the select/lock buttons and the LCD screen are only available on the larger models, not on the Launchkey Mini.)
