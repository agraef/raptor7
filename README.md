...
In order to control a single Raptor instance instead, click the unlabeled button in the top left corner of the panel. The button turns blue to indicate that the instance was selected and is now receiving all control data. At most one instance can be selected at any one time, but you can switch instances at any time, and clicking the blue button in the selected instance again will switch Raptor back to omni control mode, in which all instances receive the control data.
